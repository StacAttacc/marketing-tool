...
type SortBy = 'date' | 'amountCents' | 'channel'
type SortOrder = 'asc' | 'desc'
type SearchField = 'channel' | 'date' | 'budgetPeriod'

export function useFilteredListSpends() {
  const orpc = useOrpc()
  const page = ref(1)
  ...
  const queryOptions = computed(() =>
    orpc.spend.filter.queryOptions({
      queryKey: [...queryKeys.spends, 'list', search.value, searchField.value, sortBy.value, sortOrder.value, page.value, pageSize.value],
      input: {
